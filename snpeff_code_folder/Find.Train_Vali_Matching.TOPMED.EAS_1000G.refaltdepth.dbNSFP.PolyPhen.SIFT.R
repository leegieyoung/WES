...
colnames(Geno_refalt) <- gsub("TN","refalt_TN",colnames(Geno_refalt))
...
#		refalt <- c(refalt, Geno[,A][[B]])
#	}
#	alt <- alt[-1]
#	refalt <- refalt[-1]
#	Geno[,A] <- alt
#	Geno_refalt[,A] <- refalt
#	alt <- ""
#	refalt <- ""
#}
...
colnames(refalt) <- gsub("TN","refalt_TN",colnames(refalt))
...
#Train SNP information 추가
Vali <- alt[,1:19]
Vali_refalt <- refalt[,1:19]
Vali <- cbind(Info,TOPMED, EAS_1000G, Vali, Vali_refalt)
#해당rsID에 alt 내 depth 가 존재한느 샘플 찾기
...
#Vali <- Vali[,c(1:6,8, ncol(Vali))]

write.table(Train ,"/ichrogene/project/temp/gylee/1.WES/2.gatk_result/total69/4.VariantFiltration/Find.Train_Vali_Matching/dbNSFP.refaltdepth.vcf.Train", col.names=T, row.names=F, sep='\t', quote=F)
write.table(Vali ,"/ichrogene/project/temp/gylee/1.WES/2.gatk_result/total69/4.VariantFiltration/Find.Train_Vali_Matching/dbNSFP.refaltdepth.vcf.Vali", col.names=T, row.names=F, sep='\t', quote=F)
...
write.table(result ,"/ichrogene/project/temp/gylee/1.WES/2.gatk_result/total69/4.VariantFiltration/Find.Train_Vali_Matching/dbNSFP.refaltdepth.vcf.merge", col.names=T, row.names=F, sep='\t', quote=F)
