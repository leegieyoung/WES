...
#ref depth 뽑기
ref <- matrix(nrow=nrow(Geno), ncol=ncol(Geno))
for ( A in 1:ncol(Geno)){
        temp <- Geno[,A]
        temp2 <- unlist(lapply(str_split(temp,","), `[`,1))
        ref[,A] <- temp2
}
colnames(ref) <- colnames(Geno)
...
merge <- cbind(Info,TOPMED, EAS_1000G, Geno, N_Article_Match_samples, N_Vali_Match_samples, N_Control_Match_samples)
...
write.table(result ,"Find/dbNSFP.CADD.clinvar.refaltdepth.vcf.merge", col.names=T, row.names=F, sep='\t', quote=F)
...
write.table(QC.result ,"Find/QC.merge", col.names=T, row.names=F, sep='\t', quote=F)
